<template>
    <a-menu id="navigation-component" mode="inline" :default-selected-keys="menu.defaultSelectedKeys">
        <a-menu-item v-for="item in menu.menu" v-if="item.menu.length <= 0" :key="item.key">
            <nuxt-link :to="item.url">
                <a-icon :type="item.icon" /><span>{{ item.label }}</span>
            </nuxt-link>
        </a-menu-item>
        ...
        <a-sub-menu v-else :key="item.key">
            ...
            <a-menu-item v-for="subItem in item.menu" :key="subItem.key">
                <nuxt-link :to="subItem.url">
                    <a-icon :type="subItem.icon" /><span>{{ subItem.label }}</span>
                </nuxt-link>
            </a-menu-item>
        </a-sub-menu>
    </a-menu>
